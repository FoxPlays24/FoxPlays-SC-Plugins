...
void tellmsg(CBasePlayer@ plr, const string msg) { g_PlayerFuncs.SayText(plr, msg + "\n"); }

void NetworkBeam(Vector start, Vector end, string sprite = "sprites/laserbeam.spr", 
                 uint8 frameStart = 0, uint8 frameRate = 100, uint8 life = 10, 
                 uint8 width = 32, uint8 noise = 1, RGBA c = PURPLE, 
                 uint8 scroll = 32) {
...
void Kill(CBasePlayer@ plr, CBaseEntity@ target) {
  for (uint i = 0; i < 8; i++)
    NetworkBeam(target.pev.origin+Vector(0, 0, 4096), target.pev.origin+
                Vector(Math.RandomFloat(-20, 20), Math.RandomFloat(-20, 20), 0), 
                beamSprite, 10*i, 100, 1+i/2, 255, 20, RGBA(120, 255, 255), 128);
